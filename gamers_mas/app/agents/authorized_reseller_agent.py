...
from app.mock_data import AUTHORIZED_RESELLER_DEALS
...
class AuthorizedResellerAgent(Agent):
    class ReceiveSearchBehaviour(CyclicBehaviour):
        async def run(self) -> None:
            ...
            deals = AUTHORIZED_RESELLER_DEALS.get(resolved_title, []) if resolved_title else []
            ...
            if resolved_title:
                ...
                print(
                    f"[AuthorizedResellerAgent] Ambiguous game title '{game_title}'. Suggestions: {match_result['suggestions']}"
                )
            else:
